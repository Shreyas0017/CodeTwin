    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: codetwin mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in codetwin.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .codetwin/ subdirectory too)
  const candidates = [path.join(baseDir, "codetwin.json"), path.join(baseDir, "codetwin.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".codetwin", "codetwin.json"), path.join(baseDir, ".codetwin", "codetwin.jsonc"))
  }
  ...
  // Default to codetwin.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., codetwin x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "codetwin-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
