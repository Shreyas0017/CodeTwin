...
const WORKFLOW_FILE = ".github/workflows/codetwin.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/ct summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent - https://codetwin.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.codetwin.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  codetwin:
    if: |
      contains(github.event.comment.body, ' /ct') ||
      startsWith(github.event.comment.body, '/ct') ||
      contains(github.event.comment.body, ' /codetwin') ||
      startsWith(github.event.comment.body, '/codetwin') ||
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.codetwin.ai" : "https://codetwin.ai"
      ...
      try {
        ...
        console.log("codetwin session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.codetwin.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/codetwin,/ct,/opencode,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to codetwin...")
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[codetwin session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the codetwin infrastructure after your response",
