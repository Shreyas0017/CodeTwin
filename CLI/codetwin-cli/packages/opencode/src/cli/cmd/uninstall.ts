...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall codetwin and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CodeTwin")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g codetwin-ai",
      pnpm: "pnpm uninstall -g codetwin-ai",
      bun: "bun remove -g codetwin-ai",
      yarn: "yarn global remove codetwin-ai",
      brew: "brew uninstall codetwin",
      choco: "choco uninstall codetwin",
      scoop: "scoop uninstall codetwin",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "codetwin", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CodeTwin!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# codetwin") || content.includes(".codetwin/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".codetwin/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".codetwin/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".codetwin"))
    ) {
      continue
    }
