...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run codetwin",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for codetwin session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found codetwin session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["codetwin", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting codetwin...")
        UI.println()

        const codetwinArgs = sessionId ? ["-s", sessionId] : []
        const codetwinProcess = Process.spawn(["codetwin", ...codetwinArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await codetwinProcess.exited
        if (code !== 0) throw new Error(`codetwin exited with code ${code}`)
      },
    })
  },
})
