...
export function FormatError(input: unknown) {
  if (MCP.Failed.isInstance(input))
    return `MCP server "${input.data.name}" failed. Note, codetwin does not support MCP authentication yet.`
  ...
  if (Provider.ModelNotFoundError.isInstance(input)) {
    ...
    return [
      ...
      `Try: \`codetwin models\` to list available models`,
      `Or check your config (codetwin.json) provider/model names`,
    ].join("\n")
  }
