...
export namespace Config {
  ...
  export function managedConfigDir() {
    return process.env.CODETWIN_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }

  const managedDir = managedConfigDir()

  const MANAGED_PLIST_DOMAIN = "ai.codetwin.managed"
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.codetwin/command/", "/.codetwin/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.codetwin/agent/", "/.codetwin/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  function globalConfigFile() {
    const candidates = ["codetwin.jsonc", "codetwin.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "codetwin.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "codetwin.jsonc"))),
          )
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const dir of unique(directories)) {
            if (dir.endsWith(".codetwin") || dir === Flag.OPENCODE_CONFIG_DIR) {
              for (const file of ["codetwin.json", "codetwin.jsonc"]) {
          ...
          if (process.env.CODETWIN_CONFIG_CONTENT) {
            const source = "OPENCODE_CONFIG_CONTENT"
            const next = yield* loadConfig(process.env.CODETWIN_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            })
          ...
          if (activeOrg) {
            yield* Effect.gen(function* () {
              ...
              if (Option.isSome(tokenOpt)) {
                process.env["CODETWIN_CONSOLE_TOKEN"] = tokenOpt.value
