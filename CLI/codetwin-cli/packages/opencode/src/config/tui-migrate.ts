...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from codetwin.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const codetwinConfigFiles = await codetwinFiles(input)
  for (const file of codetwinConfigFiles) {
...
async function codetwinFiles(input: { directories: string[]; managed: string }) {
